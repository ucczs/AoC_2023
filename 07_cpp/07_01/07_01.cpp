...
enum class CardType { One, Two, Three, Four, Five, Six, Seven, Eight, Nine, Ten, Jack, Queen, King, Ace, Invalid };
...
void CardHand::analyzeHand() {
    ...
    std::vector<int> sameCardsCount;
    int sameCounter = 0;
    for (CardType card : sortedCards) {
        if (prevCard == card) {
            sameCounter++;
        } else if (sameCounter != 0) {
            sameCardsCount.push_back(sameCounter + 1);
            sameCounter = 0;
        }
        prevCard = card;
    }
    ...
    std::sort(sameCardsCount.begin(), sameCardsCount.end());

    if (sameCardsCount.size() == 0) {
        handType = HandType::HighCard;
    } else if (sameCardsCount.size() == 1) {
        if (sameCardsCount[0] == 2) {
            handType = HandType::Pair;
        } else if (sameCardsCount[0] == 3) {
            handType = HandType::ThreeOfAKind;
        } else if (sameCardsCount[0] == 4) {
            handType = HandType::FourOfAKind;
        } else if (sameCardsCount[0] == 5) {
            handType = HandType::FiveOfAKind;
        } else {
            std::cout << "INVALID handType detected" << std::endl;
            handType = HandType::Invalid;
        }
    } else if (sameCardsCount.size() == 2) {
        if (sameCardsCount[0] == 2 && sameCardsCount[1] == 3) {
            handType = HandType::FullHouse;
        } else if (sameCardsCount[0] == 2 && sameCardsCount[1] == 2) {
    ...
    while (std::getline(file, line)) {
        ...
        for (std::string::size_type i = 0; i < hand.size(); ++i) {
            if (std::isdigit(hand[i])) {
                CardType card = static_cast<CardType>(hand[i] - '1');
                cards[i] = card;
            } else {
                CardType card;
                switch (hand[i]) {
