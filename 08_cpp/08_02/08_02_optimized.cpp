...
auto split_string(const std::string full_string, const char seperator) -> std::vector<std::string> {
...
std::vector<std::string> getAllStartingNodes(const network_type& network) {
...
bool allNodesEndWithZ(const std::vector<std::string>& node_names, std::vector<int>& steps_count_reaching_z, int step_count) {
    bool valid = true;
    int count = 0;
...
std::vector<node_targets> getAllTargets(network_type& network, const std::vector<std::string>& current_nodes) {
    std::vector<node_targets> targets;
    for (const std::string& current_node : current_nodes) {
        targets.push_back(network[current_node]);
    }
    return targets;
}

std::vector<std::string> getNextNodes(const char direction, const std::vector<node_targets>& targets) {
    std::vector<std::string> next_nodes;
    for (const node_targets target : targets) {
        if (direction == 'L') {
