...
private:
    ...
    int card_count;
    ScratchCard* nextCard;
    int score;
public:
    ScratchCard(std::vector<int> winning_numbers, std::vector<int> card_numbers);
    void calculateScore();
    void setNextCard(ScratchCard* nextCard);
    void spreadWonCards();
    void incrementCardCountBy(int count);
    ScratchCard* getNextCard();
    int getCardCount();
};

void ScratchCard::setNextCard(ScratchCard* nextC){
    nextCard = nextC;
}

int ScratchCard::getCardCount(){
    return card_count;
}

ScratchCard* ScratchCard::getNextCard(){
    return nextCard;
}

void ScratchCard::incrementCardCountBy(int count){
    card_count += count;
}

void ScratchCard::spreadWonCards(){
    ScratchCard* currentFollowingCard = this->getNextCard();

    for (size_t i = 0; i < score; i++){
        if(currentFollowingCard != nullptr){
            currentFollowingCard->incrementCardCountBy(card_count);
            currentFollowingCard = currentFollowingCard->getNextCard();
        }
        else{
            break;
        }
...
void ScratchCard::calculateScore(){
...
int main(){
    ...
    for(ScratchCard* card : card_collection){
        card->spreadWonCards();
        card_count += card->getCardCount();
    }

    std::cout << card_count << std::endl;
